 ...
 <div class="main">
  ...
  <div class="row">
   ...
   <div><button id="add" class="sec">+ Add to chart</button> <button id="addpeer" class="sec">+ Add to peer</button>
    <button id="formbtn" class="sec">📄 Call-report view</button> <button id="leaguebtn" class="sec">🏆 League table</button> <button id="reportbtn" class="sec" disabled title="Select a single bank to generate a tear-sheet report">📋 Report</button> <button id="exportbtn" class="sec">⬇ Export</button> <button id="copylink" class="sec" title="Copy link to current chart state">🔗 Link</button> <button id="kbdbtn" class="sec" title="Keyboard shortcuts (?)">⌨</button></div>
  </div>
  ...
  <div><label>Measures (click items in the left rail; ✕ to remove)</label> <button id="clrmeas" class="sec" style="padding:1px 6px;font-size:12px;opacity:0.6" title="Remove all measures">✕ Clear</button> <button id="calcbtn" class="sec" style="padding:1px 6px;font-size:12px" title="Define a custom calculated series combining existing line items">Σ Calc</button>
   ...
   <div id="calcdiv" style="display:none;margin:4px 0 8px;padding:8px 10px;border:1px solid var(--border,#ccc);border-radius:6px;font-size:13px;background:var(--bg2,#f7f9fb)">
    ...
    <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;align-items:center">
     <button id="calcSave" class="sec" style="font-size:13px;padding:2px 8px" title="Save formulas to this browser (and the local server if running)">Save formulas</button>
     <button id="calcLoad" class="sec" style="font-size:13px;padding:2px 8px" title="Restore saved formulas">Load formulas</button>
  ...
  <div class="slider" id="sliderwrap" style="display:none">
   ...
   <label class="muted" title="divide each $ series by total assets (COMB2170 = RCFD or RCON, covers all charter types), producing a % ratio"><input type="checkbox" id="normbyassets"> ÷ assets</label>
...
let _idxBase=null;
let _lblDrag=null;
window._inlineLbls=false;window._lblBoxLeft=null;window._lblBoxTop=null;window._lblBoxW=0;window._lblBoxH=0;
try{window._inlineLbls=localStorage.getItem('ffiec_call_inlinelbls')==='1';window._lblBoxLeft=+localStorage.getItem('ffiec_call_lblboxleft')||null;window._lblBoxTop=+localStorage.getItem('ffiec_call_lblboxtop')||null;window._lblBoxW=+localStorage.getItem('ffiec_call_lblboxw')||0;window._lblBoxH=+localStorage.getItem('ffiec_call_lblboxh')||0;}catch(_){}
...
async function saveFormulas(){const json=getFormulasJson();const el=document.getElementById('calcstatus');
 ...
 try{const r=await fetch('/api/save-formulas',{method:'POST',headers:{'Content-Type':'application/json'},body:json});if(!r.ok)throw new Error('HTTP '+r.status);
   try{localStorage.setItem('ffiec_call_formulas',json);}catch(_){}if(el)el.textContent='';showToast('Formulas saved.','ok');return;}
 catch(e){try{localStorage.setItem('ffiec_call_formulas',json);if(el)el.textContent='';showToast('Formulas saved to this browser.','ok');}
   catch(e2){if(el)el.textContent='Save failed: '+e2.message;}}}
async function loadFormulas(){try{const r=await fetch('/api/formulas');if(!r.ok)throw new Error('HTTP '+r.status);const obj=await r.json();const n=applyFormulas(obj);showToast('Loaded '+n+' formula'+(n===1?'':'s')+'.','ok');}catch(e){try{const s=localStorage.getItem('ffiec_call_formulas');const obj=s?JSON.parse(s):{};const tot=Object.keys(obj).filter(k=>k.startsWith('CALC_')).length;applyFormulas(obj);if(tot>0)showToast('Restored '+tot+' formula'+(tot===1?'':'s')+' from this browser.','ok');else showToast('No saved formulas to load.','ok');}catch(le){showToast('No saved formulas to load.','ok');}}}
async function autoLoadFormulas(){
 ...
 try{const s=localStorage.getItem('ffiec_call_formulas');if(s)applyFormulas(JSON.parse(s));}catch(e){}}
...
async function recompute(){
 if(!measures.length||!active.length){lastSeries=[];Qall=[];draw();return;}
 const mySeq=++_rcSeq;
 ...
 _assetRows.clear();for(const e of active){if(mySeq!==_rcSeq)return;const ar=await seriesFor(e.id,'COMB2170');_assetRows.set(e.id,Object.fromEntries(ar.map(r=>[r[0],r[1]])));}
...
function draw(){const host=document.getElementById('panes');
 window._hybridOnChart=measures.some(m=>DERIV[m.code]?.type==='hybrid_sum');
...
function _getLinkTfm(){return{idx:!!(document.getElementById('idx')?.checked),qoqdelta:!!(document.getElementById('qoqdelta')?.checked),normByAssets:!!(document.getElementById('normbyassets')?.checked),stacked:!!(document.getElementById('stackedmode')?.checked)};}
function _applyLinkedTfm(tfm){['idx','qoqdelta','normbyassets','stackedmode'].forEach(id=>{const el=document.getElementById(id);if(el)el.checked=!!(id==='normbyassets'?tfm.normByAssets:id==='stackedmode'?tfm.stacked:tfm[id]);});_extraCharts.forEach(c=>{c.transforms={...c.transforms,...tfm};});}
...
async function perFilerValues(measCode, quarters){
 // map: quarter -> Map(entity_id -> per-bank value) for a COMB/raw code OR a DERIV ratio/sum
...
async function buildReport(entityId,nm,latestQ,qtrs){
 const kpiCodes=['RCFD2170','RCON2170','RCFD2122','RCON2122','RCFD2200','RCON2200',
  ...
  'RIAD4635','RIAD4605','RCFD3123','RCFD1403','RCFD1406','RCFD1407',
 ...
 const coff=getV(['RIAD4635']),rec=getV(['RIAD4605']),alll=getV(['RCFD3123']);
 const npl30=getV(['RCFD1403']),npl90=getV(['RCFD1406']),nona=getV(['RCFD1407']);
 ...
 const nplPrev=prevQ?((getV(['RCFD1403'],prevQ)||0)+(getV(['RCFD1406'],prevQ)||0)+(getV(['RCFD1407'],prevQ)||0)):null;
 ...
 const nplQ=q=>{const l=getV(['RCFD2122','RCON2122'],q);const np=(getV(['RCFD1403'],q)||0)+(getV(['RCFD1406'],q)||0)+(getV(['RCFD1407'],q)||0);return l&&l>0?100*np/l:null;};
...
(function(){
  ...
  function showTip(e,svg){
    ...
    if(window._hybridOnChart){const _rn=[];if(measures.some(m=>m.code==='COMBHK05')&&q>='2011-03-31'&&q<='2016-12-31')_rn.push('RC-E M.1.c from K220±5.5%');if(measures.some(m=>m.code==='RCFN2200')&&q>='1983-06-30'&&q<='1983-12-31')_rn.push('RC 13.b from 6636');if(_rn.length)html+=`<div style="font-size:11px;color:#64748b;margin-top:2px;font-style:italic">⚠ Reconstructed: ${_rn.join('; ')}</div>`;}
